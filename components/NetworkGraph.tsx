...
import { 
  BibEntry, GraphNode, NodeSizeMetric, 
    ...
    <div ref={containerRef} className="flex-1 w-full h-full bg-[#fdfdfe] relative flex overflow-hidden select-none font-sans">
      ...
      <div className={`absolute top-0 right-0 h-full w-[380px] bg-white border-l border-slate-100 shadow-2xl transition-transform duration-500 z-50 flex flex-col ${isPanelOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex border-b border-slate-50">
            {[{ id: 'topology', label: 'Topology', icon: '📐' }, { id: 'viz', label: 'Visuals', icon: '🎨' }, { id: 'physics', label: 'Physics', icon: '⚡' }].map(t => (
                <button key={t.id} onClick={() => setActiveTab(t.id as any)} className={`flex-1 py-5 text-[9px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${activeTab === t.id ? 'text-indigo-600 border-b-2 border-indigo-600 bg-indigo-50/30' : 'text-slate-400 hover:text-slate-600'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar">
                ...
                <div className="space-y-8 animate-fadeIn">
                    <section className="space-y-4">
                        <div className="flex justify-between items-center">
                            <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Entity Mapping</h4>
                        </div>
                        <div className="space-y-2">
                            {attributeOptions.map(opt => (
                                <label key={opt.id} className={`flex items-center gap-4 p-4 rounded-2xl cursor-pointer transition-all border ${config.selectedNodeAttrs.includes(opt.id) ? 'bg-indigo-50 border-indigo-100' : 'bg-slate-50 border-transparent hover:bg-slate-100'}`}>
                                    <input type="checkbox" checked={config.selectedNodeAttrs.includes(opt.id)} onChange={() => {
                                          const next = config.selectedNodeAttrs.includes(opt.id) ? config.selectedNodeAttrs.filter(a => a !== opt.id) : [...config.selectedNodeAttrs, opt.id];
            ...
            {activeTab === 'viz' && (
                <div className="space-y-10 animate-fadeIn">
                    <section className="space-y-4">
                        <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Layout Algorithm</h4>
                        <div className="grid grid-cols-2 gap-2">
                            {[{ id: 'force', label: 'Force 自由力导', icon: '🧲' }, { id: 'radial', label: 'Radial 径向', icon: '🔆' }, { id: 'concentric', label: 'Concentric 同心', icon: '🎯' }].map(m => (
                    ...
                    <section className="space-y-4">
                        <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Node Scaling</h4>
            ...
            {activeTab === 'physics' && (
                <div className="space-y-8 animate-fadeIn">
                    <section className="space-y-6">
                        <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Physics Controls</h4>
                        <div className="space-y-4">
                            <div className="space-y-3">
                                <div className="flex justify-between text-[10px] font-bold uppercase"><span>Link Distance</span><span className="text-indigo-600">{linkDistance}px</span></div>
      ...
      {!isPanelOpen && (
          <button onClick={() => setIsPanelOpen(true)} className="absolute top-10 right-10 w-16 h-16 bg-white text-slate-900 rounded-[1.5rem] shadow-3xl flex items-center justify-center text-3xl hover:bg-slate-900 hover:text-white transition-all z-[100] border border-slate-100 hover:scale-110 active:scale-95">⚙️</button>
      )}
    </div>
  );
};
